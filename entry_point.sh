...
jq -r '.[]' <(echo $DIDS) | while read did; do
  ...
  python src/main.py --config-name yolo_pigs images_path="/workdir/input" annotations_path="/workdir/output/$did/labels/" hydra.run.dir="/workdir/output/$did/tracking/"
  rm -r /workdir/input;
done
